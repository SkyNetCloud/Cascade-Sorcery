...
package ca.skynetcloud.cascadesorcery.world.dimension;
...
import ca.skynetcloud.cascadesorcery.procedures.ShadowRealmCanTravelThroughPortalProcedure;
import ca.skynetcloud.cascadesorcery.item.ShadowRealmItem;
...
public class ShadowRealmDimension extends CascadeSorceryModElements.ModElement {
	@ObjectHolder("cascade_sorcery:shadow_realm")
	public static final ModDimension dimension = null;
	@ObjectHolder("cascade_sorcery:shadow_realm_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public ShadowRealmDimension(CascadeSorceryModElements instance) {
		super(instance, 19);
	...
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("cascade_sorcery:shadow_realm")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("cascade_sorcery:shadow_realm"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("cascade_sorcery:shadow_realm"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("cascade_sorcery:shadow_biome")),};
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote
					&& ShadowRealmCanTravelThroughPortalProcedure.executeProcedure(ImmutableMap.of("entity", entity))) {
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == Shadow_IngotOreBlockBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == Shadow_IngotOreBlockBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Shadow_IngotOreBlockBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag
											? Shadow_IngotOreBlockBlock.block.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Shadow_IngotOreBlockBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
